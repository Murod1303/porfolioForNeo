...
const HeroBackground: React.FC<{ reduced: boolean }> = ({ reduced }) => (
  <motion.div
    animate={reduced ? {} : {
      background: [
    ...
    style={reduced ? { background: "radial-gradient(circle at 20% 50%, rgba(59, 130, 246, 0.15) 0%, transparent 50%)" } : undefined}
    transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
    className="absolute inset-0"
  />
);

const HeroShapes: React.FC<{ parallaxX: any; parallaxY: any; reduced: boolean }> = ({
  parallaxX,
  parallaxY,
  reduced,
}) => (
  <>
    ...
    <motion.div
      ...
      animate={reduced ? {} : { scale: [1.2, 1, 1.2], opacity: [0.5, 0.3, 0.5] }}
...
const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    ...
    >
      ...
      <HeroShapes parallaxX={parallaxX} parallaxY={parallaxY} reduced={reduced} />
